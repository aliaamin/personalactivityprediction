...
data = read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv", na.strings=c("NA","","#DIV/0!"), sep=",")
val_data = read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv", na.strings=c("NA","","#DIV/0!"), sep=",")
```
...
After dataset clean up, we found there are 52 predictors columns and the result ("classe" column). 
```{r warning=FALSE,  message=FALSE}
...
featurePlot(x=training[,1:3], y=training[,53], plot="density", scales=list(x=list(relation="free"), y=list(relation="free")), layout = c(3, 1))

featurePlot(x=training[,49:52], y=training[,53], plot="density", scales=list(x=list(relation="free"), y=list(relation="free")), layout = c(3, 1))
...
We also try several other machine learning method to see if we are able to increase the accuracy of our prediction, including the estimated out of sample error, as seen in each one round of cross-validation.
...
```{r warning=FALSE,  message=FALSE}
x <- matrix(c(a$overall['Accuracy'],b$overall['Accuracy'],c$overall['Accuracy'],a$overall['Kappa'],b$overall['Kappa'],c$overall['Kappa']),ncol=2,nrow=3,byrow=TRUE)
cbind(x,)
